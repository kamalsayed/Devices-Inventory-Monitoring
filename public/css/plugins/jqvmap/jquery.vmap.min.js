...
var VectorCanvas = function (a, b, c) {
  if (
    ((this.mode = window.SVGAngle ? 'svg' : 'vml'), (this.params = c), 'svg' ===
      this.mode)
  )
  ...
  'svg' === this.mode
    ...
    : ((this.canvas = this.createVmlNode (
        'group'
      )), (this.canvas.style.position = 'absolute')), this.setSize (a, b);
};
...
var ColorScale = function (a, b, c, d) {
  a && this.setColors (a), b && this.setNormalizeFunction (b), c &&
    this.setMin (c), c && this.setMax (d);
};
...
var JQVMap = function (a) {
  ...
  (this.selectedRegions = []), (this.multiSelectRegion =
    a.multiSelectRegion), (this.container = a.container), (this.defaultWidth =
    d.width), (this.defaultHeight = d.height), (this.color =
    a.color), (this.selectedColor = a.selectedColor), (this.hoverColor =
    a.hoverColor), (this.hoverColors = a.hoverColors), (this.hoverOpacity =
    a.hoverOpacity), this.setBackgroundColor (
    a.backgroundColor
  ), (this.width = a.container.width ()), (this.height = a.container.height ()), this.resize (), jQuery (
    window
  ).resize (function () {
    var d = a.container.width (), e = a.container.height ();
    if (d && e) {
      (c.width = d), (c.height = e), c.resize (), c.canvas.setSize (
        c.width,
        c.height
      ), c.applyTransform ();
      var f = jQuery.Event ('resize.jqvmap');
      jQuery (a.container).trigger (f, [d, e]), b &&
        (jQuery ('.jqvmap-pin').remove (), (c.pinHandlers = !1), c.placePins (
          b.pins,
          b.mode
        ));
    }
    ...
    this.canvas.canvas
  ), this.makeDraggable (), (this.rootGroup = this.canvas.createGroup (
    !0
  )), (this.index = JQVMap.mapIndex), (this.label = jQuery ('<div/>')
    ...
    (jQuery ('<div/>')
      ...
      .text ('+')
      .appendTo (a.container), jQuery ('<div/>')
      .addClass ('jqvmap-zoomout')
      .html ('&#x2212;')
      .appendTo (a.container)), (c.countries = []);
  for (var e in d.paths) {
    var f = this.canvas.createPath ({
      path: d.paths[e].path,
    });
    f.setFill (this.color), (f.id = c.getCountryId (e)), (c.countries[
      e
    ] = f), 'svg' === this.canvas.mode
      ? f.setAttribute ('class', 'jqvmap-region')
      : jQuery (f).addClass ('jqvmap-region'), jQuery (this.rootGroup).append (
      f
    );
  }
  if (
    (jQuery (a.container).delegate (
      ...
      function (b) {
        ...
        (f = f.toLowerCase ()), 'mouseover' === b.type
          ? (jQuery (a.container).trigger (h, [
              f,
              d.paths[f].name,
            ]), h.isDefaultPrevented () || c.highlight (f, e), a.showTooltip &&
              (c.label.text (d.paths[f].name), jQuery (
                a.container
              ).trigger (g, [c.label, f]), g.isDefaultPrevented () ||
                (c.label.show (), (c.labelWidth = c.label.width ()), (c.labelHeight = c.label.height ()))))
          : (c.unhighlight (f, e), c.label.hide (), jQuery (
              a.container
            ).trigger ('regionMouseOut.jqvmap', [f, d.paths[f].name]));
      }
    ), jQuery (a.container).delegate (
      'svg' === this.canvas.mode ? 'path' : 'shape',
      'click',
      function (b) {
        ...
        if (
          ((f = f.toLowerCase ()), jQuery (a.container).trigger (g, [
            f,
            d.paths[f].name,
          ...
          for (var h in d.paths)
            (c.countries[h].currentFillColor = c.countries[
              h
            ].getOriginalFill ()), c.countries[h].setFill (
      ...
      a.container.mousemove (function (a) {
        if (c.label.is (':visible')) {
          ...
          0 > b && (b = a.pageX + 15), 0 > d &&
            (d = a.pageY + 15), c.label.css ({
            left: b,
            top: d,
          });
        }
      }), this.setColors (a.colors), this.canvas.canvas.appendChild (
      this.rootGroup
    ), this.applyTransform (), (this.colorScale = new ColorScale (
      a.scaleColors,
      a.normalizeFunction,
      a.valueMin,
      a.valueMax
    )), a.values &&
      ((this.values = a.values), this.setValues (a.values)), a.selectedRegions)
  )
  ...
  if (
    (this.bindZoomButtons (), a.pins &&
      ((b = {
        pins: a.pins,
        mode: a.pinMode,
      }), (this.pinHandlers = !1), this.placePins (
        a.pins,
        a.pinMode
      )), a.showLabels)
  ) {
...
(JQVMap.prototype = {
  ...
  'http://www.w3.org/1999/xlink'), (JQVMap.mapIndex = 1), (JQVMap.maps = {}), (function () {
  var a = {
    colors: 1,
    values: 1,
    backgroundColor: 1,
    ...
    if ('addMap' === c) JQVMap.maps[arguments[1]] = arguments[2];
      ...
      if ('set' !== c || !a[arguments[1]]) {
        ...
        jQuery.extend (d, c), (d.container = this), this.css ({
          position: 'relative',
          overflow: 'hidden',
        }), (e = new JQVMap (d)), this.data ('mapObject', e), this.unbind (
          '.jqvmap'
        );
        for (var f in b)
  ...
  (this.clearMaxValue = a), 'function' == typeof this.normalize
    ? (this.maxValue = this.normalize (a))
    : (this.maxValue = a);
}), (ColorScale.prototype.setMin = function (a) {
  (this.clearMinValue = a), 'function' == typeof this.normalize
  ...
  'polynomial' === a
    ...
    : 'linear' === a
        ...
        : (this.normalize = a), this.setMin (this.clearMinValue), this.setMax (
    this.clearMaxValue
  );
  ...
  this.defaultWidth * this.scale <= this.width
    ? ((a =
        (this.width - this.defaultWidth * this.scale) / (2 * this.scale)), (c =
        (this.width - this.defaultWidth * this.scale) / (2 * this.scale)))
    : ((a = 0), (c =
        (this.width - this.defaultWidth * this.scale) / this.scale)), this
    .defaultHeight *
    this.scale <=
    this.height
    ? ((b =
    ...
    : ((b = 0), (d =
        (this.height - this.defaultHeight * this.scale) / this.scale)), this
    .transY > b
    ? (this.transY = b)
    : this.transY < d && (this.transY = d), this.transX > a
    ? (this.transX = a)
    : this.transX < c && (this.transX = c), this.canvas.applyTransformParams (
    this.scale,
    this.transX,
    this.transY
  );
  ...
  if (
    ((a = a.toLowerCase ()), (b =
      b || jQuery ('#' + this.getCountryId (a))[0]), this.isSelected (a))
  )
    this.selectedRegions.splice (this.selectIndex (a), 1), jQuery (
      this.container
    ).trigger ('regionDeselect.jqvmap', [
      a,
    ]), (b.currentFillColor = b.getOriginalFill ()), b.setFill (
      b.getOriginalFill ()
    );
  else
    for (var c in this.countries)
      this.selectedRegions.splice (
        this.selectedRegions.indexOf (c),
        1
      ), (this.countries[c].currentFillColor = this.color), this.countries[
        c
      ].setFill (this.color);
  ...
  (b = b || jQuery ('#' + this.getCountryId (a))[0]), this.hoverOpacity
    ? b.setOpacity (this.hoverOpacity)
    : this.hoverColors && a in this.hoverColors
        ? ((b.currentFillColor = b.getFill () + ''), b.setFill (
            this.hoverColors[a]
          ))
        : this.hoverColor &&
            ((b.currentFillColor = b.getFill () + ''), b.setFill (
  ...
  this.container
    .mousemove (function (e) {
      return c &&
        ((d.transX -= (a - e.pageX) / d.scale), (d.transY -=
          (b - e.pageY) / d.scale), d.applyTransform (), (a = e.pageX), (b =
          e.pageY), (d.isMoving = !0), d.isMovingTimeout &&
          clearTimeout (d.isMovingTimeout), d.container.trigger ('drag')), !1;
    })
    .mousedown (function (d) {
      return (c = !0), (a = d.pageX), (b = d.pageY), !1;
    })
    .mouseup (function () {
      return (c = !1), clearTimeout (d.isMovingTimeout), (d.isMovingTimeout = setTimeout (
        function () {
          d.isMoving = !1;
        },
        100
    ...
    if (1 === n.length) {
      if (1 === e) {
        ...
        (l = d.transX), (m = d.transY), (d.transX -=
          (j - n[0].pageX) / d.scale), (d.transY -=
          (k - n[0].pageY) / d.scale), d.applyTransform (), (l !== d.transX ||
          m !== d.transY) &&
          a.preventDefault (), (d.isMoving = !0), d.isMovingTimeout &&
  ...
  var c = this;
  if (
    ((!b || ('content' !== b && 'id' !== b)) && (b = 'content'), 'content' === b
      ? jQuery.each (a, function (a, b) {
          if (0 !== jQuery ('#' + c.getCountryId (a)).length) {
            var d = c.getPinId (a), e = jQuery ('#' + d);
            e.length > 0 && e.remove (), c.container.append (
              '<div id="' +
                d +
                '" for="' +
                a +
  ...
  this.width / this.height > this.defaultWidth / this.defaultHeight
    ? ((this.baseScale = this.height / this.defaultHeight), (this.baseTransX =
        Math.abs (this.width - this.defaultWidth * this.baseScale) /
        (2 * this.baseScale)))
    : ((this.baseScale = this.width / this.defaultWidth), (this.baseTransY =
        Math.abs (this.height - this.defaultHeight * this.baseScale) /
        (2 * this.baseScale))), (this.scale *=
    this.baseScale / a), (this.transX *= this.baseScale / a), (this.transY *=
    this.baseScale / a);
  ...
  (a = a.toLowerCase ()), (b =
    ...
    (this.multiSelectRegion
      ...
      : (this.selectedRegions = [a]), jQuery (
      this.container
    ).trigger ('regionSelect.jqvmap', [a]), this.selectedColor &&
      b &&
      ((b.currentFillColor = this.selectedColor), b.setFill (
        this.selectedColor
      )));
  ...
  if ('string' == typeof a)
    this.countries[a].setFill (b), this.countries[a].setAttribute (
      'original',
      b
    );
  else {
    ...
    for (var d in c)
      this.countries[d] &&
        (this.countries[d].setFill (c[d]), this.countries[d].setAttribute (
          'original',
          c[d]
        ));
  }
}), (JQVMap.prototype.setNormalizeFunction = function (a) {
  this.colorScale.setNormalizeFunction (a), this.values &&
  ...
  for (var e in a)
    (e = e.toLowerCase ()), (b = parseFloat (a[e])), isNaN (b) ||
      (b > c && (c = a[e]), d > b && (d = b));
  d === c && c++, this.colorScale.setMin (d), this.colorScale.setMax (c);
  var f = {};
  for (e in a)
    (e = e.toLowerCase ()), (b = parseFloat (a[e])), (f[e] = isNaN (b)
      ? this.color
      : this.colorScale.getColor (b));
  this.setColors (f), (this.values = a);
}), (JQVMap.prototype.unhighlight = function (a, b) {
  (a = a.toLowerCase ()), (b =
    b || jQuery ('#' + this.getCountryId (a))[0]), b.setOpacity (
    1
  ...
  if (a.zoomCurStep < a.zoomMaxStep) {
    (a.transX -=
      (a.width / a.scale - a.width / (a.scale * a.zoomStep)) / 2), (a.transY -=
      (a.height / a.scale - a.height / (a.scale * a.zoomStep)) /
      2), a.setScale (a.scale * a.zoomStep), a.zoomCurStep++;
    var c = jQuery ('#zoomSlider');
    c.css ('top', parseInt (c.css ('top'), 10) - b), a.container.trigger (
      'zoomIn'
    );
  }
  ...
  if (a.zoomCurStep > 1) {
    (a.transX +=
      (a.width / (a.scale / a.zoomStep) - a.width / a.scale) / 2), (a.transY +=
  ...
  'svg' === this.mode
    ...
    : ((this.rootGroup.coordorigin =
        this.width - b + ',' + (this.height - c)), (this.rootGroup.coordsize =
        this.width / a + ',' + this.height / a));
}), (VectorCanvas.prototype.createGroup = function (a) {
  var b;
  return 'svg' === this.mode
    ? (b = this.createSvgNode ('g'))
    : ((b = this.createVmlNode ('group')), (b.style.width =
        this.width + 'px'), (b.style.height =
        this.height + 'px'), (b.style.left = '0px'), (b.style.top =
        '0px'), (b.coordorigin = '0 0'), (b.coordsize =
        this.width + ' ' + this.height)), a && (this.rootGroup = b), b;
}), (VectorCanvas.prototype.createPath = function (a) {
  var b;
  if ('svg' === this.mode)
    (b = this.createSvgNode ('path')), b.setAttribute ('d', a.path), null !==
      this.params.borderColor &&
      b.setAttribute ('stroke', this.params.borderColor), this.params
      .borderWidth > 0 &&
      (b.setAttribute (
        'stroke-width',
        this.params.borderWidth
      ), b.setAttribute ('stroke-linecap', 'round'), b.setAttribute (
        'stroke-linejoin',
        'round'
      ...
      this.setAttribute ('fill', a), null === this.getAttribute ('original') &&
    ...
    (b = this.createVmlNode ('shape')), (b.coordorigin = '0 0'), (b.coordsize =
      this.width + ' ' + this.height), (b.style.width =
      this.width + 'px'), (b.style.height = this.height + 'px'), (b.fillcolor =
      JQVMap.defaultFillColor), (b.stroked = !1), (b.path = VectorCanvas.pathSvgToVml (
      a.path
    ));
    var c = this.createVmlNode ('skew');
    (c.on = !0), (c.matrix = '0.01,0,0,0.01,0,0'), (c.offset =
      '0,0'), b.appendChild (c);
    var d = this.createVmlNode ('fill');
    b.appendChild (d), (b.setFill = function (a) {
      (this.getElementsByTagName ('fill')[0].color = a), null ===
        this.getAttribute ('original') && this.setAttribute ('original', a);
    }), (b.getFill = function () {
    ...
    .replace (/([MmLlHhVvCcSs])((?:-?(?:\d+)?(?:\.\d+)?,?\s?)+)/g, function (
      ...
      switch (g) {
        ...
        case 'V':
          ...
          break;
        case 'c':
          (b = e + h[h.length - 4]), (c = f + h[h.length - 3]), (e +=
            h[h.length - 2]), (f += h[h.length - 1]), (d = 'v' + h.join (','));
          break;
        case 'C':
          (b = h[h.length - 4]), (c = h[h.length - 3]), (e =
            h[h.length - 2]), (f = h[h.length - 1]), (d = 'c' + h.join (','));
          break;
        case 's':
          h.unshift (f - c), h.unshift (e - b), (b = e + h[h.length - 4]), (c =
            f + h[h.length - 3]), (e += h[h.length - 2]), (f +=
            h[h.length - 1]), (d = 'v' + h.join (','));
          break;
        case 'S':
          h.unshift (f + f - c), h.unshift (e + e - b), (b =
            h[h.length - 4]), (c = h[h.length - 3]), (e = h[h.length - 2]), (f =
            h[h.length - 1]), (d = 'c' + h.join (','));
      }
      return d;
    })
    .replace (/z/g, '');
  ...
  if ('svg' === this.mode)
    this.canvas.setAttribute ('width', a), this.canvas.setAttribute (
      'height',
      b
    );
  else if (
    ((this.canvas.style.width = a + 'px'), (this.canvas.style.height =
      b + 'px'), (this.canvas.coordsize =
      a + ' ' + b), (this.canvas.coordorigin = '0 0'), this.rootGroup)
  ) {
    for (
      ...
      (c[d].coordsize = a + ' ' + b), (c[d].style.width = a + 'px'), (c[
        d
      ].style.height =
        b + 'px');
    (this.rootGroup.coordsize = a + ' ' + b), (this.rootGroup.style.width =
      a + 'px'), (this.rootGroup.style.height = b + 'px');
  }
