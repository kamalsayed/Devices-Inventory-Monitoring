        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title" >طابعات ألوان</h3>
                    </div>
                    <!-- /.card-header -->
                    <div class="card-body">
                ...
                <table  class="table table-bordered table-striped" id="colored_printer">
...
<script>
    ...
    $(document).ready(function(){
                ...
                if(printer_brand !='' && working !='' && serial !='' && place !=''){
                    $.ajax({
                        ...
                        success:function(data){
                            ...
                            $('#serial').html("");
                            $('#place').html("");
                            $('#colored_printer').DataTable();
                            
                            //table =$('#colored_printer').DataTable();
        ...
        $(document).on('blur','.column_name',function(){
            ...
            if(column_value!=''){
                $.ajax({
                    url:"{{url('/showcolored/update_data')}}",
                    method:'post',
        ...
        $(document).on('click','.delete',function(){
            ...
            if(confirm("هل أنت متأكد انك تريد حذف ذلك الصف ؟")){
                $.ajax({
                    url: "{{url('/showcolored/delete_data')}}",
                    method:"POST",
        ...
        function fetch_data(){
         $.ajax({
                 type: 'get',
                 url:"/showcolored/fetch_data",
                 dataType:"json",
                 success:function(data){
                   
                    $('#trick').empty();
                    $('#trick').html(`<table  class="table table-bordered table-striped" id="colored_printer">
                    ...
                    $('#colored_printer tbody tr:not(:first)').empty(); 
                    if(table!=null){
                    table.clear();
                    table.distroy();
                    table =$('#colored_printer').DataTable();
                    ...
                    html+='<td contenteditable class="column_name" data-column_name="working" data-id="'+data[i].id+'">يعمل</td>';
                       ...
                       $('#colored_printer tbody').html(html);
                       var table =$('#colored_printer').DataTable();
                  ...
                  $("#colored_printer tbody").append(tr_str);
                  var table =$('#colored_printer').DataTable();
                }
                }
